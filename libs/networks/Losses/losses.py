...
class Losses():
    '''
    A loss container which can store the losses during trainging.
    And provide some methods which reduce the backward precedure
    '''
...
def compute_rpn_bbox_loss(target_bbox, rpn_match, rpn_bbox):
    ...
    rpn_bbox: [batch * anchors, (dy, dx, log(dh), log(dw))]
            Since each image has different number of positive boxes,
            rpn_bbox.size()[0] is the sum of all box for all images.
    """
    # Squeeze last dim to simplify
    rpn_match = rpn_match.squeeze(2)
...
def compute_mrcnn_class_loss(target_class_ids, pred_class_logits):
    ...
    It's single image avaliable. Batch loop call this function.
    (Since each image has different number of roi.)
    ...
    if target_class_ids.nelement() != 0 and pred_class_logits.nelement() != 0:
        ...
        loss = torch.tensor([0], dtype=torch.float32, device=pred_class_logits.device)
...
def compute_mrcnn_bbox_loss(target_bbox, target_class_ids, pred_bbox):
    ...
    It's single image avaliable. Batch loop call this function.
    (Since each image has different number of roi.)
    The tail of the target will be filled with 0 for batch training.
    ...
    if target_class_ids.nelement() != 0:
        ...
        loss = torch.tensor([0], dtype=torch.float32, device=pred_bbox.device)
...
def compute_mrcnn_mask_loss(target_masks, target_class_ids, pred_masks):
    ...
    target_masks: [num_rois, height, width].
        A float32 tensor of values 0 or 1. Uses zero padding to fill array.
    target_class_ids: [num_rois]. Integer class IDs. Zero padded.
    pred_masks: [proposals, num_classes, height, width] float32 tensor
    ...
    if target_class_ids.nelement() != 0:
        ...
        loss = torch.tensor([0], dtype=torch.float32, device=pred_masks.device)
...
def compute_rpn_losses(rpn_target, rpn_out):
    ...
    zero = torch.tensor([0.0], dtype=torch.float32, device=rpn_out.deltas.device)
...
def compute_mrcnn_losses(mrcnn_targets, mrcnn_outs):
    zero = torch.tensor([0.0], dtype=torch.float32, device=mrcnn_outs[0].deltas.device)
