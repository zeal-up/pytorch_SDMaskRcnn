...
from libs.networks.tensor_container.image_metas import ImageMetas
from libs.networks.tensor_container.detection_output import DetectionOutput
...
def subtract_mean(images):
    ...
    colors in RGB order.
    """
    return images.astype(np.float32) - Config.DATASET.IMAGE.MEAN_PIXEL
...
def mold_image(image):
    ...
    molded_image, image_metas = resize_image(
        image,
        min_dim=Config.DATASET.IMAGE.MIN_DIM,
        max_dim=Config.DATASET.IMAGE.MAX_DIM,
        min_scale=Config.DATASET.IMAGE.MIN_SCALE,
        mode=Config.DATASET.IMAGE.RESIZE_MODE,
        input_shape=Config.DATASET.IMAGE.SHAPE)
...
def unmold_boxes(boxes, class_ids, masks, image_metas, scores=None):
    """
    Reformats the detections of one image from the format of the neural
    network output to the input image format.
    

    boxes: [N, (y1, x1, y2, x2)]
    class_ids: [N]
    masks: [N, height, width]
    images_metas: original_shape[4], window[4], scale[2], padding[6], crop[4], image_id[1]
    scores: [N]
...
def to_img_domain(boxes, image_metas):
    '''
    Transform a boxes of the padded and resize image coordinates to the
